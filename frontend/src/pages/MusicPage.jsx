...
export default function MusicPage() {
  ...
  return (
    <div className="page-container max-w-5xl">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="section-title flex items-center gap-2">
            <span className="material-symbols-outlined text-[#00FFB2] text-2xl">music_note</span>
            Music
          </h1>
      ...
      {/* Info banner */}
      <div className="bg-[#00FFB2]/5 border border-[#00FFB2]/15 rounded-xl px-4 py-3 mb-6
        flex items-center gap-2 text-xs text-[#00FFB2]">
      ...
      <div className="glass-card p-6 mb-6">
        <div className="text-sm font-bold text-on-surface mb-1 flex items-center gap-2">
          <span className="material-symbols-outlined text-[#00FFB2] text-lg">headphones</span>
          Ambient Sounds
        </div>
        ...
        <div className="flex gap-2.5 flex-wrap">
          {AMBIENT_SOUNDS.map(a=>(
            <button key={a.id} onClick={()=>{handleAmbient(a.id);sfx.click();}}
              ...
              style={{
                background:ambient===a.id?"rgba(0,255,178,.08)":"rgba(255,255,255,.03)",
                border:`1px solid ${ambient===a.id?"rgba(0,255,178,.25)":"rgba(255,255,255,.05)"}`,
                color:ambient===a.id?"#00FFB2":"#4a5568"
              }}>
              ...
              {a.label}
              {ambient===a.id&&a.id!=="off"&&<span className="w-1.5 h-1.5 rounded-full bg-[#00FFB2] animate-pulse"/>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className="glass-card p-6">
            <span className="material-symbols-outlined text-2xl text-[#00FFB2] mb-3 block">{t.icon}</span>
