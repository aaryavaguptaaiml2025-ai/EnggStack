    ...
    <div className="page-container flex flex-col items-center gap-8">
      ...
      <div className="glass-card flex p-1 gap-1">
        {Object.entries(LBL).map(([m,label])=>(
          <button key={m} onClick={()=>{sfx.click();sw(m);}}
            ...
            style={{background:mode===m?color+"15":"transparent",
              border:`1px solid ${mode===m?color+"55":"transparent"}`,
      ...
      <div className="relative w-[264px] h-[264px] flex items-center justify-center">
        <svg width={264} height={264} className="absolute" style={{transform:"rotate(-90deg)"}}>
          <circle cx={132} cy={132} r={R} fill="none" stroke="rgba(255,255,255,.06)" strokeWidth={10}/>
          ...
          <div className="flex gap-1.5 justify-center mt-2">
            {[0,1,2,3].map(i=>(
              <div key={i} className="w-2 h-2 rounded-full transition-all duration-200"
                style={{background:i<(sess%4)?color:"rgba(255,255,255,.08)",
      ...
      <div className="flex items-center gap-5">
        ...
        <button onClick={()=>{sfx.click();setRun(r=>!r);}}
          ...
          style={{background:color,boxShadow:`0 0 22px ${color}44`}}>
      ...
      {/* Session stats */}
      <div className="glass-card px-14 py-6 text-center">
