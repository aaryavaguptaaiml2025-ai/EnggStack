...
<<<<<<< HEAD
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center",
      ...
      <div style={{ position:"absolute", width:400, height:400, borderRadius:"50%",
        background:"radial-gradient(circle,rgba(96,165,250,.05),transparent 70%)",
        bottom:"-10%", left:"-10%", pointerEvents:"none" }}/>
      <div className="fade-up" style={{ width:"100%", maxWidth:420, zIndex:1 }}>
        <div style={{ textAlign:"center", marginBottom:28 }}>
          <div style={{ width:60, height:60, background:"var(--ac)", borderRadius:16,
            display:"inline-flex", alignItems:"center", justifyContent:"center",
            fontSize:28, fontWeight:900, color:"#000", marginBottom:14,
            boxShadow:"0 0 24px var(--ac-dim)" }}>E</div>
        ...
        <div style={{ background:"var(--card)", border:"1px solid var(--border)",
          borderRadius:20, padding:26, boxShadow:"0 20px 48px rgba(0,0,0,.4)" }}>
=======
...
function GoogleBtn({ onSuccess, label = "Continue with Google" }) {
  ...
  // Read Google Client ID — tries Vite env var first, then window variable
  const getClientId = () => {
    const fromEnv    = import.meta.env.VITE_GOOGLE_CLIENT_ID;
    const fromWindow = window.__GOOGLE_CLIENT_ID__;

    if (fromEnv && !fromEnv.includes("YOUR"))    return fromEnv;
    if (fromWindow && !fromWindow.includes("YOUR")) return fromWindow;
    return null;
  };
  ...
  const handleClick = () => {
    ...
    if (!window.google) {
      ...
      s.onerror = () => { setBusy(false); alert("Could not load Google Sign-In. Check your internet connection."); };
    ...
    try {
      ...
      window.google.accounts.id.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          ...
          const div = document.getElementById("google-btn-fallback");
          if (div) {
            window.google.accounts.id.renderButton(div, { theme:"outline", size:"large", width:380 });
          }
        }
      });
    } catch (e) {
  ...
  return (
    <>
      <button onClick={handleClick} disabled={busy} style={{
        ...
        borderRadius:10, padding:"11px 16px", color:"var(--text)", fontSize:13, fontWeight:500,
        cursor: busy ? "default" : "pointer",
        display:"flex", alignItems:"center", justifyContent:"center", gap:10,
        transition:"all .15s", marginBottom:14, opacity: busy ? 0.7 : 1,
      }}
        onMouseEnter={e => { if (!busy) e.currentTarget.style.background="rgba(255,255,255,.1)"; }}
        onMouseLeave={e => e.currentTarget.style.background="rgba(255,255,255,.06)"}
      >
        {busy ? (
          <div style={{ width:18, height:18, border:"2px solid rgba(255,255,255,.3)", borderTop:"2px solid var(--text)", borderRadius:"50%", animation:"spin .7s linear infinite" }}/>
        ) : (
      ...
      {/* Fallback container for rendered Google button */}
      <div id="google-btn-fallback" style={{ display:"flex", justifyContent:"center", marginBottom:8 }}/>
    </>
  );
}
...
<<<<<<< HEAD
      ...
      <div style={{ display:"flex", background:"var(--bg2)", borderRadius:10, padding:3, gap:3, marginBottom:14 }}>
        {["email","pin"].map(t => (
          ...
          }}>{t === "pin" ? "PIN Login" : "Email / Password"}</button>
        ))}
      </div>
      ...
      {tab === "pin" && (
        <>
          ...
          <div style={{ fontSize:11, color:"var(--dim)", textAlign:"center", marginTop:8 }}>Set your PIN in Settings after first login</div>
        </>
      )}
...
<<<<<<< HEAD
      ...
      {err && <div style={{ color:"#f87171", fontSize:12, marginTop:8 }}>{err}</div>}
      <Btn full color="var(--ac)" onClick={() => go(register, name, email, pass, uname||undefined)} disabled={loading} style={{marginTop:14}}>
        {loading ? <Spinner color="#000"/> : "Create Account"}
      </Btn>
      <div style={{ textAlign:"center", fontSize:12, color:"var(--muted)", marginTop:14 }}>
