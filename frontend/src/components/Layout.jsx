  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      ...
      <div className="absolute -bottom-[10%] left-[30%] w-[35%] h-[35%]
        bg-purple/5 blur-[100px] rounded-full" style={{animation:"floatOrb3 26s ease-in-out infinite"}}/>
    </div>
  );
}
...
function Navbar({ onMenuToggle }) {
  ...
  return (
    <header className="h-16 bg-bg/60 backdrop-blur-xl border-b border-white/5
      flex items-center justify-between px-6 sticky top-0 z-30">
      {/* Left: mobile menu + breadcrumb */}
      <div className="flex items-center gap-4">
        <button onClick={onMenuToggle}
          className="md:hidden p-2 text-dim hover:text-on-surface transition-colors">
      ...
      <div className="flex items-center gap-3">
        <button className="p-2 text-dim hover:text-primary hover:bg-white/5
        ...
        <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-primary/20
          ...
          {avatar
            ...
            : <div className="w-full h-full bg-purple/10 flex items-center justify-center
                text-xs font-bold text-purple">{initials}</div>
          }
        </div>
      </div>
    </header>
